...
export default function Index() {
  ...
  const fetchHabits = async () => {
    try {
      const response = await database.listDocuments<Habit>(
        DB_ID!,
        HABITS_COLLECTION_ID!,
  ...
  const handleDeleteHabit = async (id: string) => {
    try {
      await database.deleteDocument(DB_ID, HABITS_COLLECTION_ID, id);
    } catch (error) {
      console.error(error);
    }
  };

  const handleCompletionsHabit = async (id: string) => {
    if (!user) return;
    try {
      ...
      await database.updateDocument(DB_ID, HABITS_COLLECTION_ID, id, {
            ...
            <Swipeable
              ...
              onSwipeableOpen={(direction) => {
                if (direction === "left") {
                  ...
                  handleCompletionsHabit(habit.$id);
                }
